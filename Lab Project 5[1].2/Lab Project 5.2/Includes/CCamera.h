...
public:
    ...
    virtual void        SetPosition      ( const D3DXVECTOR3& Position ) { m_vecPos = Position; m_bViewDirty = true; }
    virtual void        Move             ( const D3DXVECTOR3& vecShift ) { m_vecPos += vecShift; m_bViewDirty = true; }
...
//-----------------------------------------------------------------------------
// Name : CCamFPSFly (Class)
// Desc : Camera object which exhibits first person shooter style behaviour. 
//-----------------------------------------------------------------------------
class CCamFPSFly : public CCamera
{
public:
    //-------------------------------------------------------------------------
    // Constructors & Destructors for This Class.
    //-------------------------------------------------------------------------
    CCamFPSFly( const CCamera * pCamera );
    CCamFPSFly();
