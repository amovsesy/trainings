...
LRESULT CALLBACK GameApplication::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		break;
	case WM_KEYDOWN:
		if ( !GameApplication::pApp->KeyDown ( static_cast<long>(wParam), static_cast<long>(lParam) ) )
			if ( !GameApplication::pApp->ProcessMessage ( msg, wParam, lParam ) )
				return DefWindowProc ( hwnd, msg, wParam, lParam ); 
		break;
	case WM_KEYUP:
		if ( !GameApplication::pApp->KeyUp ( static_cast<long>(wParam), static_cast<long>(lParam) ) )
			if ( !GameApplication::pApp->ProcessMessage ( msg, wParam, lParam ) )
				return DefWindowProc ( hwnd, msg, wParam, lParam ); 
		break;
	case WM_MOUSEMOVE:
