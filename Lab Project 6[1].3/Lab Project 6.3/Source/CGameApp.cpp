...
bool CGameApp::CreateDisplay()
{
    ...
    LPTSTR          WindowTitle  = _T("Scene Textures");
    USHORT          Width        = 400;
    USHORT          Height       = 400;
    RECT            rc;
...
void CGameApp::SetupGameState()
{
    ...
    m_Player.SetCamOffset     ( D3DXVECTOR3( 0.0f, 15.0f, 0.0f ) );
    m_Player.SetCamLag        ( 0.0f ); // No camera lag
    m_Player.SetPosition      ( D3DXVECTOR3( 0.0f, -15.0f, 0.0f ) );
...
void CGameApp::SetupRenderStates()
{
    ...
    m_pD3DDevice->SetRenderState( D3DRS_AMBIENT, 0x5D5D5D );
    ...
    m_Scene.SetTextureFormat( m_TextureFormat );
...
bool CGameApp::BuildObjects()
{
    ...
    m_pD3D->GetDeviceCaps( pSettings->AdapterOrdinal, pSettings->DeviceType, &Caps );
    ...
    // Load our scene data
    if (!m_Scene.LoadScene( _T("Data\\Colony5.iwf"), LightLimit, 1 )) return false;
...
void CGameApp::FrameAdvance()
{
    ...
    if ( m_LastFrameRate != m_Timer.GetFrameRate() )
    {
        ...
        _stprintf( TitleBuffer, _T("Scene Textures : %s"), FrameRate );
    ...
    m_pD3DDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0 ); //0x79D3FF
...
bool CMyD3DInit::ValidateDevice( const D3DDEVTYPE &Type, const D3DCAPS9 &Caps )
{
    ...
    // Must support 'Modulate' texture op.
    if ( !(Caps.TextureOpCaps & D3DTEXOPCAPS_MODULATE ) ) return false;
