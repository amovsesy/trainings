...
void CGameApp::SetupRenderStates()
{
    ...
    m_Terrain.SetRenderMode( m_bSinglePass );
...
bool CGameApp::TestDeviceCaps( )
{
    ...
    Enable = MF_ENABLED;
    if ( !(Value & D3DTEXOPCAPS_MODULATE) ) Enable = MF_DISABLED | MF_GRAYED;
    EnableMenuItem( m_hMenu, ID_COLOROP_MODULATE, MF_BYCOMMAND | Enable );
    if ( Enable == MF_ENABLED ) m_ColorOpCaps[D3DTOP_MODULATE] = true;

    // Determine if 'Add' op is supported
    Enable = MF_ENABLED;
    if ( !(Value & D3DTEXOPCAPS_ADD) ) Enable = MF_DISABLED | MF_GRAYED;
    EnableMenuItem( m_hMenu, ID_COLOROP_ADD, MF_BYCOMMAND | Enable );
    if ( Enable == MF_ENABLED ) m_ColorOpCaps[D3DTOP_ADD] = true;

    // Determine if 'Add Signed 2x' op is supported
    Enable = MF_ENABLED;
    if ( !(Value & D3DTEXOPCAPS_ADDSIGNED2X) ) Enable = MF_DISABLED | MF_GRAYED;
    EnableMenuItem( m_hMenu, ID_COLOROP_ADDSIGNED2X, MF_BYCOMMAND | Enable );
    if ( Enable == MF_ENABLED ) m_ColorOpCaps[D3DTOP_ADDSIGNED2X] = true;

    // Determine if 'Modulate 4x' op is supported
    Enable = MF_ENABLED;
    if ( !(Value & D3DTEXOPCAPS_MODULATE4X) ) Enable = MF_DISABLED | MF_GRAYED;
    EnableMenuItem( m_hMenu, ID_COLOROP_MODULATE4X, MF_BYCOMMAND | Enable );
    if ( Enable == MF_ENABLED ) m_ColorOpCaps[D3DTOP_MODULATE4X] = true;

    // Determine if 'Subtract' op is supported
    Enable = MF_ENABLED;
    if ( !(Value & D3DTEXOPCAPS_SUBTRACT) ) Enable = MF_DISABLED | MF_GRAYED;
    EnableMenuItem( m_hMenu, ID_COLOROP_SUBTRACT, MF_BYCOMMAND | Enable );
...
void CGameApp::SelectMenuItems()
{
    ...
    // Select which color-op menu item is selected
    switch ( m_ColorOp )
    {
        case D3DTOP_ADDSIGNED:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_ADDSIGNED, MF_BYCOMMAND );
            break;
        case D3DTOP_MODULATE2X:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_MODULATE2X, MF_BYCOMMAND );
            break;
        case D3DTOP_MODULATE:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_MODULATE, MF_BYCOMMAND );
            break;
        case D3DTOP_ADD:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_ADD, MF_BYCOMMAND );
            break;
        case D3DTOP_ADDSIGNED2X:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_ADDSIGNED2X, MF_BYCOMMAND );
            break;
        case D3DTOP_MODULATE4X:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_MODULATE4X, MF_BYCOMMAND );
            break;
        case D3DTOP_SUBTRACT:
            ::CheckMenuRadioItem( m_hMenu, ID_COLOROP_ADDSIGNED, ID_COLOROP_MODULATE4X, ID_COLOROP_SUBTRACT, MF_BYCOMMAND );
            break;

    } // End Switch

    // Select which render mode menu item is selected
    switch ( m_bSinglePass )
    {
        case true:
            ::CheckMenuRadioItem( m_hMenu, ID_RENDERMODE_SINGLEPASS, ID_RENDERMODE_MULTIPASS, ID_RENDERMODE_SINGLEPASS, MF_BYCOMMAND );
            break;
        case false:
            ::CheckMenuRadioItem( m_hMenu, ID_RENDERMODE_SINGLEPASS, ID_RENDERMODE_MULTIPASS, ID_RENDERMODE_MULTIPASS, MF_BYCOMMAND );
            break;
    ...
    // Disable color op menu if using multi-pass
    ULONG Enable = MF_ENABLED;
    if ( m_bSinglePass == false ) Enable = MF_DISABLED | MF_GRAYED;
    HMENU SubMenu = GetSubMenu( m_hMenu, 2 );
    EnableMenuItem( SubMenu, 5, MF_BYPOSITION | Enable );
	...
	switch (Message)
    {
        ...
        case WM_COMMAND:
            ...
            switch( LOWORD(wParam) )
            {
                ...
                case ID_COLOROP_ADDSIGNED:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_ADDSIGNED;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_COLOROP_MODULATE2X:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_MODULATE2X;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_COLOROP_MODULATE:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_MODULATE;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_COLOROP_MODULATE4X:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_MODULATE4X;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;
                ...
                case ID_COLOROP_ADDSIGNED2X:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_ADDSIGNED2X;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_COLOROP_SUBTRACT:
                    // Set alternate colorop
                    m_ColorOp  = D3DTOP_SUBTRACT;
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_RENDERMODE_SINGLEPASS:
                    // Set alternate render mode
                    m_bSinglePass = true;
                    m_Terrain.SetRenderMode( m_bSinglePass );
                    SetupRenderStates(); // Called here to allow state code centralization
                    break;

                case ID_RENDERMODE_MULTIPASS:
                    // Set alternate render mode
                    m_bSinglePass = false;
                    m_Terrain.SetRenderMode( m_bSinglePass );
                    SetupRenderStates(); // Called here to allow state code centralization
...
void CGameApp::FrameAdvance()
{
    ...
    // Skybox must ALWAYS be rendered before anything
    RenderSkyBox();
    
    // Render player before terrain, because terrain may render alpha components
...
void CGameApp::RenderSkyBox( )
{
    ...
    if ( m_bSinglePass ) m_pD3DDevice->SetTextureStageState( 1, D3DTSS_COLOROP, m_ColorOp );
}
