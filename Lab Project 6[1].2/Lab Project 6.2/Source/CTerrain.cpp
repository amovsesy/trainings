...
bool CTerrain::LoadHeightMap( LPCTSTR FileName, ULONG Width, ULONG Height )
{
    ...
    hRet = D3DXCreateTextureFromFile( m_pD3DDevice, BaseTextureName, &m_pBaseTexture );
    if ( FAILED(hRet) ) return false;

    hRet = D3DXCreateTextureFromFile( m_pD3DDevice, DetailTextureName, &m_pDetailTexture );
    if ( FAILED(hRet) ) return false;
